...
Если localhost.run выдал новый адрес, можно не перезаливать `config.js`, а открыть сайт с параметром `api`. Сайт запомнит новый backend в браузере:

```text
https://ВАШ_ЛОГИН.github.io/ВАШ_РЕПОЗИТОРИЙ/admin.html?api=https://new-address.lhr.life
https://ВАШ_ЛОГИН.github.io/ВАШ_РЕПОЗИТОРИЙ/index.html?api=https://new-address.lhr.life
```

Эту новую ссылку можно отправить студентам. Если домен туннеля снова поменялся, меняется только значение после `api=`.

## Вариант 3: нормальный бесплатный режим через Firebase

Это самый стабильный вариант без своего ПК и без меняющихся ссылок.

1. Откройте `https://console.firebase.google.com/`.
2. Создайте проект.
3. В меню выберите `Build` -> `Realtime Database`.
4. Нажмите `Create Database`.
5. Выберите режим тестирования или задайте правила вручную.
6. Скопируйте адрес базы. Он выглядит примерно так:

```text
https://your-project-default-rtdb.europe-west1.firebasedatabase.app
```

7. В файле `config.js` вставьте адрес в `FIREBASE_DB_URL`:
...
8. Загрузите обновленные файлы на GitHub Pages.

После этого:

- админка сохраняет тесты в Firebase;
- все студенты сразу видят одинаковые тесты;
- результаты сохраняются в Firebase в разделе `results`;
- компьютер можно выключить, сайт продолжит работать.

Для учебной демонстрации можно использовать правила из файла `firebase-rules.json`. После сдачи проекта лучше закрыть запись или удалить базу.
